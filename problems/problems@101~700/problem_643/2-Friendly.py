...
def Mertens(D):
    ...
    return mobius, M_list


def euler_sum(N, Imax, mobius, M_list):
    D = N // Imax
    ...
    # compute S2
    s2 = Mxi_sum - Imax * (Imax - 1) // 2 * M_list[D]
    return s1 + s2
...
def solve(N):
    N = N // 2
    I = int(N**(1 / 3))
    mobius, M_list = Mertens(N // I)

    res = 0
    while N > 1:
        tmp = euler_sum(N, I, mobius, M_list) % 1000000007
        print(tmp)
        res += tmp - 1
        res %= 1000000007
